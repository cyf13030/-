...
st.title("个人基金估值查询")
st.caption(f"数据来源：东方财富估算净值 (AKShare) | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
if not selected_funds:
    ...
    with st.spinner("正在获取东方财富实时估算数据..."):
        try:
            df = ak.fund_value_estimation_em(symbol="全部")
            df['基金代码'] = df['基金代码'].astype(str).str.zfill(6)
            
            # 调试输出：显示实际列名
            st.caption("调试：当前 AKShare 返回的列名（用于排查）")
            ...
            if not value_col or not growth_col:
                st.error("列名匹配失败（接口可能更新）。请把上方列名列表复制给我，我帮你调整。")
            else:
